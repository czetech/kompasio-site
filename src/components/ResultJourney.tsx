...
const ResultJourney: Component = (props) => {
  ...
  const hitStepTitle = props.item._highlightResult.steps.find(
    (item) => item.title.matchLevel !== "none",
  )?.title;
  let hitStepBody;
  ...
  return (
    <a
      class="text-vibrant-blue rounded-3xl border p-6"
      href={`/zivotne-situacie/${props.item.slug}`}
    >
      <h2
        class="mb-4 text-2xl"
      ...
      <Show
        when={
          hitStepTitle &&
          !(props.item._highlightResult.title.matchLevel !== "none") &&
          !hitDescription
        }
      >
      ...
      <div class="flex pt-4">
        <p class="text-shuttle-white bg-vibrant-blue rounded-full px-2 text-sm">
          {props.item.category}
        </p>
      </div>
    </a>
